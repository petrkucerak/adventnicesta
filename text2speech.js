    ...
    fs.readdir("_days/", (err, filenames) => {
      ...
      filenames.forEach((filename) => {
        fs.readFile(`_days/${filename}`, "utf-8", (err, content) => {
          ...
          let text = `Vítej u dnešního zamyšlení na Tvojí Adventní cestě!\nDnes je ${contentJSON.dayName} a autorem zamyšlení je ${contentJSON.author}.\n\nÚryvek z Bible\n${contentJSON.quote}\n\nZamyšlení\n${contentJSON.reflexion}\n\nZávěrečná modlitba\n${contentJSON.preayer}`;
          // console.log(contentJSON.slug + text);
          SynthesizeSpeech(text, contentJSON.slug, SPEECH_KEY, LOCATION_REGION);
          setTimeout(() => {
            console.log(contentJSON.slug);
          }, 4000);
        });
      });
    });
